...
def _get_triplet_mask(labels: torch.Tensor) -> bool:
...
def batch_all_triplet_loss(labels: torch.Tensor, embeddings: torch.Tensor,
                            margin: float, squared=False) -> torch.Tensor:
    """Build the triplet loss over a batch of embeddings.
    ...
    num_triplet = torch.sum(mask)
    print(f"number of triplet: {num_triplet}")
...
if __name__ == '__main__':
    ...
    labels = [10000, 10001, 10002, 10003, 20004, 10005, 10006, 10007, 10008,
            ]
